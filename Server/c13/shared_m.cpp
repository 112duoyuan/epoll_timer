...
int run_child(int idx,client_data*users,char*share_mem){
    ...
    /*子进程使用I/O复用技术来同时监听两个文件描述符：客户连接socket、与父进程通信
    的管道文件描述符*/
    ...
    while(!stop_child){
        ...
        for(int i=0;i<number;i++){
            ...
            }else if((sockfd==pipefd)&&(events[i].events&EPOLLIN)){//主进程通知本进程（通过管道）将第client个客户的数据发送到本进程负责的客户端
                int client=0;
        ...
        for(int i=0;i<number;i++){
            ...
            if(sockfd==listenfd){
                ...
                if(ret==-1){
                    ...
                    /*向除负责处理第child个客户连接的子进程之外的其他子进程发送消息，通知它们有客
                    户数据要写*/
